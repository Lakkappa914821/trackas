...
const ClassSchedule = () => {
  ...
  console.log("DEBUG → useUserDetails returned:", userDetails);
  ...
  if (!userDetails) {
  return <p className="text-center mt-10">Loading lecturer data...</p>;
}
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ CHECK LECTURER ID BEFORE INSERT
    if (!lecturerId) {
      toast.error("Login error: Lecturer ID missing. Please sign in again.");
      console.error("DEBUG → lecturerId missing, userDetails:", userDetails);
      return;
    }
    ...
    const registrationLink = `${VERCEL_URL}/attendance?courseCode=${encodeURIComponent(
    courseCode
    ...
    // ⭐ DEBUG: Check logged-in lecturerId
    console.log("Lecturer ID used for insert:", lecturerId);
    ...
    if (error) {
      ...
      const generatedCourseId = data[0]?.id;
